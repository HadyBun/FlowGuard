...
export default function ReceivablesPage() {
  ...
  async function load() {
    setLoading(true)
    try {
      const data = await getReceivables(filter === 'ALL' ? undefined : filter)
      setItems(data)
    } catch { toast.error('Gagal load piutang.') }
  ...
  async function onSubmit(data) {
    setSubmitting(true)
    try {
      const newItem = await addReceivable({
  ...
  async function handleMarkOverdue(id) {
    try {
      const updated = await updateReceivableStatus(id, 'OVERDUE')
      setItems(prev => prev.map(i => i.id === id ? updated : i))
  ...
  return (
    <div className="page-enter">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 600 }}>Receivables</h1>
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 3 }}>Tagihan piutang dari klien</p>
        </div>
          ...
          <table>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} style={{ textAlign: 'center', padding: 48, color: 'var(--text-muted)' }}>Belum ada data piutang</td></tr>
              ) : (
                displayed.map(item => {
                  const isOverdue = item.status === 'PENDING' && isPast(parseISO(item.due_date))
                  return (
                    <tr key={item.id}>
                      <td style={{ color: 'var(--text-primary)', fontWeight: 500 }}>{item.client_name}</td>
                      ...
                      <td style={{ textAlign: 'right', fontFamily: 'var(--font-mono)', fontWeight: 500, color: 'var(--accent)' }}>
                        {fmt(item.amount)}
                      </td>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 20 }}>
          <div className="card" style={{ width: '100%', maxWidth: 400, padding: 28 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <h2 style={{ fontSize: 16, fontWeight: 600 }}>Add Receivable</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="form-group">
                ...
                <input className="input" placeholder="e.g. PT Nebula Dynamics"
                  {...register('client_name', { required: 'Nama klien wajib diisi' })} />
                {errors.client_name && <p className="error-msg">{errors.client_name.message}</p>}
              </div>
              <div className="form-group">
